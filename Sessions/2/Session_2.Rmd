...
A basic task in the analysis of RNA-seq count data is to detect differentially expressed genes (DEGs). A gene is differentially expressed if the change in the read counts between two experimental conditions is statistically significant. It is possible to perform differential expression analysis with DESeq2 package. Its differential expression tests are based on a negative binomial generalized linear model.
...
The first step is importing data. An RNA-seq count data should be a non-normalized sequence read counts at either the gene or transcript level are presented as a table. The rows are the detected genes and the columns are the number of sequence fragments that have been assigned to each gene for each sample.
...
The next step is to create an object of class DESeqDataSet which will store the counts and intermediate calculations we need for the differential expression analysis. We use `DESeqDataSetFromMatrix(couvst_dfata, colData, design`) for this aim. The first argument is the couvst_dfata, The input should be an matrix, including integer values. Let's take a look at our count data.
...
# round the values to get rid of decimal part
...
The third argument is design. The DESeqDataSet object store the design formula used to estimate dispersion and log2 fold changes used within the model. When specifying the formula it should take the form of a "\~" followed by "+" separating factors.
...
You can see dds stores the count data in assays, design formula in the design, and the colData in the colData. It is not possible to access to the count data directly from a DESeqDataSet object. We need to use `assay()`.
...
Genes with very low counts doesn't lead to reliable differential expression results. We need to do some light pre-filtering. This will reduce the size of the DEseq2DataSet object and speed up the runtime of the algorithm.
...
We use `DESeq()` to perform differential expression analysis. It performs some steps, including an outlier removal procedure. This function performs a default analysis thought these below steps:
...
The estimates of differences between sample groups calculated by `DESeq()` are not corrected for expression level. It means when counts are small, it leads to some large differences between sample groups. We can correct this by applying a "shrinkage" procedure.

For this aim, we use `lfcShrink()`, but first we need to know the name and/or the position of the "coefficient"[\^1](Coefficient%20is%20the%20number%20in%20front%20of%20a%20variable%20or%20term.%20For%20example,%20in%20%606a%20+%201b%20+%202xy%60%206,%201,%202%20are%20the%20coefficients.) that is calculated by `DESeq()`. We can use `resultsNames()`. The `resultsNames()` returns the names of the estimated effects (coefficients) of the model.
...
Here we describes what does each column mean:
...
The `summary()` helps us to see the results briefly. You can set the `alpha` argument to 0.05. This is the value of adjusted p-value.
...
Adjusted p-values can be set to NA values if all samples have zero counts within a row, if a row contains a sample with an extreme count outlier, or if a row is filtered because it has a low mean normalized count.
...
We can name the genes and we can add a theme layer to the plot.
...
Before visualizing count data, we need to transform it. The variance stabilizing transformations or VST is the most accurate and fastest way.
...
We are going to use `pheatmap() from pheatmap package` to plot static heatmap. We are going to visualize top 30 genes, based on adjusted p-value.

```{r, fig.show='hide', include=TRUE}
# sort shrunken_res by adjusted p-value acsending
shrunken_res <- shrunken_res[order(shrunken_res$padj),]

# select top 15 up regulated
top_15_up <- subset(shrunken_res, shrunken_res$padj <= 0.05 & shrunken_res$log2FoldChange > 0)
top_15_up <- top_15_up[1:15, ]
# select top 15 down regulated
...
Let's change the color of heatmap. Here are some of color pallets in RColorBrewer package.
...
We selected the new color for the plot. Let's add an annotation bar for the samples.
...
To plot the interactive heatmap, we are using `d3heatmap()` from d3heatmap package. First, we need to cluster the rows and columns.
...
PCA is an unsupervised method, similar to clustering, to find patterns. It simplifies the complexity in high-dimensional data. You can perform PCA with `prcomp()`.
