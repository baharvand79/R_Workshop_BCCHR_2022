...
knitr::opts_chunk$set(echo = TRUE) #highlite = github
```

# Introduction to the Workshop

This workshop is for biologists interested in learning how to work with R to analyze their bulk RNA-seq data and perform pathway analysis. This will be a code-along, in-person workshop.
...
RStudio is an IDE (integrated development environment) that is used to manage and execute R code.
...
You can see the phrase is presented on your console. When that little angle bracket is there, it means there is no work in progress and you can run a new instruction. But if you couldn't see the angle bracket, it means some instructions are running and you need to wait.
...
Here is a list of maths functions of base R. You can find useful functions from base R in the book of *R for Beginners* by Emmanuel Paradis on page 36 and 37.
...
It created using the c(). There are different ways to create a vector. The below tables shows them.
...
**Q:** Is it possible to store different types of variable in a vector?

it is possible, but after creating the vector, the variables' types changes.
...
You can name a vector elements.
...
If you have already a vector, there are two functions to use.
...
You can select elements of a vector by position and value. We call the position of the elements index.
...
You can add a vector to other with c(). You can use it to add a new element, too.
...
You can show length of a vector by length().
...
Factors are used to represent categorical data. They are kind of ordered labelled vector. Factors are an important class for statistical analysis and useful for plotting. The factor() and allows us to create factors.
...
cities_fac
# unique() returns a vector/dataframe with duplicate elements/rows removed
# we know sort() from previous part. When you sort a vector of characters, it will sort it alphabetically
```

Factor is a type of data.
...
A data frame is the most common way of sorting data in R. It is a list of equal length vectors. It is table shaped and have columns and rows.
...
|                                            |                              |
|----------------------------------------|--------------------------------|
| ![](images/paste-F09170C2.png){width="65"} | df[ , 2 ] or df\$column_name |
| ![](images/paste-C64D46A3.png){width="65"} | df [ 2 , ]                   |
| ![](images/paste-FF47D08D.png){width="65"} | df [ 2 , 2 ]                 |

When we don't specify the row index or column index, it means we are selecting all of it.
...
Syntax: matrix(x, nrow, ncol, byrow) The nrow and ncol arguments determine the number od rows and columns. The byrow argument accepts logical values. If it sets to TRUE the matrix is filled by columns, otherwise is filled by row.
...
The mentioned functions in data frame part, can be used for matrices.
...
You can create a list with list(). Syntax: list(object1, object2, object3, ...)
...
**Q_1:** When we convert a data frame or a matrix to a list, we are missing the column names. Is it possible to assign the column names to the elements of the list?
...
However, you cannot assign the column names directly. It cannot recognize the elements of each column. So we need to define the names by ourselves.
...
No, sometimes it is not possible to convert two dimensional data structures to a single dimensional data structure. For example, you cannot convert a data frame or a list to a vector.
...
You can write your own functions in order to make repetitive operations using a single command. Your function can return nothing and should be defined before using.

![](images/paste-7C17BFCE.png){width="436"}
...
-   Name of an object should be meaningful and show its purpose clearly.

-   There are different styles of naming objects, but it is recommended to use snake_case style. In this style each space is replaced by an underscore character, and the first letter of each word is written in lowercase.
...
-   Show what type of data structure is your object. For example `cars_df`, shows this object is a data frame.

-   Start the name of logical variables with "is". For example, `is_odd = TRUE`.

-   Sometimes you have to use a string or a number several times. For instance, you have a not ordered vector of numbers. You want to select numbers more than 10, but you are not sure if you want to change it later and you used this condition many times in your code. It is better to define this value as a variable. Because if you decide to change the value you don't need to change it one by one. You will change the value. It is better to name it will all uppercase letters. Like `SELECTED_NUMS = 10`.
...
-   There are some reserved names in R. For example, data.frame is a reserved name. You cannot name your objects similar to reserved name. Because they will be overwritten and you will miss them.
...
With library() we can load the packages into the session and make its functions available to use.
...
You can see some packages are loaded. But the loading message is followed by a message that some functions are masked. Because some of functions have the same name in different packages. For example, both stats and dplyr have a function called filter. To determine which package do you want to use, you can write it in this format: `dplyr::filter()`
...
You can install a package from GitHub. First, you need to install devtools package.
...
Data wrangling - also called data cleaning - refers to a variety of processes designed to transform raw data.
...
We use readr package to import data. There are four types of files.

-   delimited file: A delimited file is a sequential file with column delimiters.

-   csv file: A CSV (comma-separated values) file is a text file in which information is separated by commas or semicolons.
...
We have a file in the "Sessions/1/data/" which contains a tidy data from some women about their living place, their height and weight.
...
We will learn some functions that are helpful for transforming our data.
...
# mutate add the new column as the last column. You can change the place by .before or .after arguments.
...
The ifelse() is a shorthand vectorized alternative to the standard if...else statement. Syntax: ifelse(test_expression, x, y)
...
The case_when() is an alternative to an ifelse(). Whenever you need to use multiple if else statements, you can use it. Syntax: case_when(conditional_statement1 \~ x, conditional_statement2 \~ y, ...)
...
result <- case_when(num_vec == 0 ~ "zero",
                    num_vec %% 2 == 0 ~ "even",
                    TRUE ~ "odd") # TRUE means if not all above conditions equal to TRUE, so set "odd" as the value
names(result) <- c(0:10)
result
```
...
You can use group_by() if you want to perform functions separately on different splits of data. Syntax: group_by(data, column1, column2, ....)
...
**Q1:** Why the results are different? Because calculating the mean in grouped data frame equals to mean in each group, but in ungrouped data frame equals to mean of all weights.
...
**Q2:** Is it possible to group a data based on different variables? It is not possible with group_by(). You can add a new column shows the results of performed conditions and use that for group_by().
...
The subset() returns a subset of the object which meet conditions. Syntax: subset (x, subset_sondition)

```{r}
women_tall <- subset(women, bmi == "normal")
women_tall

women_na <- subset(women, is.na(height_cm)) # is.na() returns a logical value determines if the variable in NA
women_na
```

### distinct()

The distinct selects only unique rows from a data frame. It picks the first represented duplicate row. Syntax: distinct(df, column1, column2, ..., .keep_all) Alwaya set .keep_all to TRUE.
...
Here is the new table. There are thre names that are not unique.
...
You can select columns from a dataframe with select().
...
"If a variable contains observations with multiple delimited values, this separates the values and places each one in its own row." Here, the city_province column values are delimited by comma. Syntax: separate_rows(data, col, ..., sep, convert, remove) The convert should be a logical value. Always set it to TRUE.
...
The separate() turns a single character column into multiple columns. Syntax: separate(data, col, into, sep, convert) The convert should be a logical value. Always set it to TRUE.
...
The unite() paste together multiple columns into one. Syntax: unite(data, col, column1, column2, ..., sep, remove)
...
The na.omit() returns the object with incomplete cases removed. Syntax: na.omit(data)
...
The drop_na() drops rows where any column we specified contains a missing value. Syntax: drop_na(data, column1, column2, ...)
...
The replace_na() replaces NAs with specified values. Syntax: replace(data, replace)
...
The column_to_rownames() convert a column to rownames. Syntax: column_to_rownames(data, var)
...
The rownames_to_column() convert rownames to a column. Syntax: rownames_to_column(data, var)
...
the "\\" is a special character in R. If you want to type an slash you need to put another slash before it. Also, if you want to have a quotation in your string, you need to put an slash before it.
